  ...
  render() {
    ...
    return (
      ...
      {display === "list"
          ...
          : items && this.getItems(items).map((item, i) =>
            this.getItems(item).map((it, j) =>
              <Card
                ...
                onClick={item => this.goToDetail(item)}
              >
                {detail && <Text className="description">
                  {item.description &&
                    item.description.replace(/<\/?[^>]+(>|$)/g, "")}
                </Text>}
                {it.details && it.details.map((itemDetail, key) => <DataList
                    key={key}
                    head={itemDetail.left}
                    data={itemDetail.right}
                  />
                )}
                {it.overview && it.overview.map((overview, key) => <TextIcon key={key} icon={overview.icon ? overview.icon : null}>
                    {overview.left}
                  </TextIcon>
                )}
                {it.videos && it.videos.map((video, key) => <Video key={key} id={video.key} type={video.site} /> )}
              </Card>
              )
        )}
      </div>
